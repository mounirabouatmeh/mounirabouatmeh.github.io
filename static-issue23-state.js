(() => {
  ...
  const s = {
    pricing: null, pricingId: null, selectedCandidateId: null, lastSentCandidateId: null,
    confirmation: null, confirmationCandidateId: null, reviewTextSeen: false, streamBusy: false,
    toolNames: new Map(), phases: Object.fromEntries(stages.map((x) => [x, "waiting"])),
  };
  const syncFns = [];
  let queued = false;
  ...
  function toolInput(tool, data) {
    if (tool === "baseline") {
      ...
      s.confirmation = null; s.confirmationCandidateId = null; s.reviewTextSeen = false;
    } else if (tool === "discovery") {
      s.phases.intent = "complete"; s.phases.discovery = "running";
    } else if (tool === "pricing") {
      s.pricing = null; s.pricingId = null; s.selectedCandidateId = null; s.lastSentCandidateId = null;
      s.confirmation = null; s.confirmationCandidateId = null; s.reviewTextSeen = false;
      Object.assign(s.phases, { pricing: "running", summary: "waiting", confirmation: "waiting", final: "waiting" });
    } else if (tool === "confirmation") {
      s.confirmationCandidateId = data?.candidateId ?? s.confirmationCandidateId; s.reviewTextSeen = false;
      Object.assign(s.phases, { summary: "complete", confirmation: "running", final: "waiting" });
    }
  }
  function toolOutput(tool, data) {
    if (!data) return;
    if (tool === "baseline" || (data.baselineId && !data.discoveryId && !data.pricingId)) s.phases.baseline = data.phase === "completed" ? "complete" : data.phase === "failed" ? "error" : "running";
    else if (tool === "discovery" || (data.discoveryId && !data.pricingId)) s.phases.discovery = data.phase === "completed" ? "complete" : data.phase === "failed" ? "error" : "running";
    else if (tool === "pricing" || (data.pricingId && Array.isArray(data.candidates))) {
      if (data.phase === "completed" && Array.isArray(data.candidates)) { s.pricing = data; s.pricingId = data.pricingId; s.phases.pricing = "complete"; s.phases.summary = s.streamBusy ? "running" : "ready"; }
      else s.phases.pricing = data.phase === "failed" ? "error" : "running";
    } else if (tool === "confirmation" || (["CONFIRMED", "EXACT_CHECK_FAILED"].includes(data.status) && data.candidateId)) {
      s.confirmation = data; s.confirmationCandidateId = data.candidateId ?? s.confirmationCandidateId; s.reviewTextSeen = false;
      s.phases.summary = "complete"; s.phases.confirmation = data.status === "CONFIRMED" ? "complete" : "error"; s.phases.final = "running";
    }
  }
  function capture(e) {
    if (!e?.type) return;
    if (e.type === "tool-input-start" && e.toolCallId) { s.toolNames.set(e.toolCallId, e.toolName); toolInput(e.toolName); }
    if (e.type === "tool-input-available" && e.toolCallId) { s.toolNames.set(e.toolCallId, e.toolName); toolInput(e.toolName, e.input); }
    if (e.type === "tool-output-available") toolOutput(e.toolName ?? s.toolNames.get(e.toolCallId), e.output);
    if (e.type === "text-delta" && s.confirmation?.status === "CONFIRMED" && String(e.delta ?? "").trim()) s.reviewTextSeen = true;
    if (e.type === "tool-output-error") { const tool = e.toolName ?? s.toolNames.get(e.toolCallId); if (tool === "confirmation") s.phases.confirmation = "error"; else if (s.phases[tool]) s.phases[tool] = "error"; }
    queue();
  }
    ...
    try {
      while (true) {
        ...
        while (true) {
          ...
          const raw = buffer.slice(0, m.index); buffer = buffer.slice(m.index + m[0].length);
      ...
      s.streamBusy = false;
      if (s.pricing?.phase === "completed" && s.phases.summary === "running" && s.phases.confirmation === "waiting") { s.phases.summary = "complete"; s.phases.confirmation = "ready"; }
      if (s.confirmation?.status === "CONFIRMED") s.phases.final = s.reviewTextSeen ? "complete" : "ready";
      globalThis.CuberenceIssue23?.releaseScrollGuard?.(); queue();
    }
  }
  globalThis.fetch = async (...args) => {
    const chat = typeof args[0] === "string" ? args[0].includes(API) : args[0]?.url?.includes(API);
    if (chat) { s.streamBusy = true; if (s.pricing?.phase === "completed" && s.phases.confirmation !== "running") s.phases.summary = "running"; queue(); }
    const response = await previousFetch(...args);
    if (chat && response.body) inspect(response.clone()).catch(() => { s.streamBusy = false; globalThis.CuberenceIssue23?.releaseScrollGuard?.(); queue(); });
    return response;
  };
  globalThis.CuberenceIssue23 = { s, stages, node, money, formatDateTime, candidateName, candidateStatus, effectiveCandidate, evaluation, visibleCandidates, syncFns, queue };
})();
